				...
				<?php while ( have_rows('gallery') ) : the_row();  ?>
					<div class="col-md-<?php echo esc_attr(get_sub_field('item_size'));?> portfolio-image">
						...
						<?php if($image_preload==''||$image_preload=='None'): ?>
							<a href="<?php echo esc_url($image[0]);?>" class="image-link">
								<span style="padding-bottom: <?php echo ($image[2] * 100) / $image[1]; ?>%; height: 0; display: block;">
									<img alt="" width="<?php echo esc_attr($image[1]);?>" height="<?php echo esc_attr($image[2]);?>" src="<?php echo esc_url($image[0]);?>" data-original="<?php echo esc_url($image[0]);?>"/>
								</span>	
							</a>
						<?php elseif($image_preload=='Lazy-Load'): ?>
							<a href="<?php echo esc_url($image[0]);?>" class="image-link">
								<span class="lazy-container" style="padding-bottom: <?php echo ($image[2] * 100) / $image[1]; ?>%; height: 0;">
									<img class="lazy" alt="" width="<?php echo esc_attr($image[1]);?>" height="<?php echo esc_attr($image[2]);?>" data-original="<?php echo esc_url($image[0]);?>"/>
								</span>
							</a>
						<?php elseif($image_preload=='Blur-Preload'): ?>
							<a href="<?php echo esc_url($image[0]);?>" class="image-link">
								<span class="blur-container" style="padding-bottom: <?php echo ($image[2] * 100) / $image[1]; ?>%; height: 0;">
									<img class="preload" alt="" src="<?php echo esc_url($small_image_url[0]);?>" data-original="<?php echo esc_url($image[0]);?>"/>
								</span>
							</a>
						<?php endif;?>
