<?php
function stonedthemes_googlemap_vc_map_init(){

	if ( class_exists( "WPBakeryShortCode" ) ) {

		class WPBakeryShortCode_sth_googlemap extends WPBakeryShortCode {
			...
			protected function content( $atts, $content = null ) {
				...
				$output .= '<div class="main-container map-contact-container '.esc_attr($fit_to_screen).'" style="position:relative;'.esc_attr($height).'">
							<div class="map-canvas" data-map data-map-options=\'{"zoom": '.esc_attr($zoom).', "title": "'.esc_attr($map_title).'", "icon": "'.esc_url($map_marker[0]).'"}\' 	data-mapStyle=\''.esc_attr($map_style).'\'>'.$coordinates_container.'</div>';
				if(!$map_desc)	{		
...
add_action('vc_after_init', 'stonedthemes_googlemap_vc_map_init');
?>